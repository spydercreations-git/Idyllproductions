...
interface FlowingMenuProps {
  ...
  borderColor?: string;
  onItemClick?: (text: string, index: number, originY: number) => void;
}
...
interface MenuItemProps extends MenuItemData {
  ...
  borderColor: string;
  isFirst: boolean;
  onClick?: (text: string, originY: number) => void;
}
      ...
      <nav className="menu">
        {items.map((item, idx) => (
          <MenuItem
            ...
            isFirst={idx === 0}
            onClick={(text, originY) => onItemClick?.(text, idx, originY)}
          />
        ))}
      </nav>
    </div>
...
const MenuItem: React.FC<MenuItemProps> = ({
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Get the center Y position of the clicked item
    const rect = itemRef.current.getBoundingClientRect();
    const originY = rect.top + rect.height / 2;
    
    onClick?.(text, originY);
  };
