# Idyll Productions - Official Website

<div align="center">
<img width="1200" height="475" alt="Idyll Productions Banner" src="https://github.com/user-attachments/assets/0aa67016-6eaf-458a-adb2-6e31a0763ed6" />
</div>

A modern, high-performance video editing portfolio website built with React, TypeScript, and **100% FREE** video hosting.

## 🚀 Features

- **Modern Design** - Clean, professional interface with smooth animations
- **Video Portfolio** - Showcase of video editing work across multiple categories
- **FREE Video Hosting** - Using GitHub + jsDelivr CDN (no costs!)
- **Responsive** - Optimized for all devices and screen sizes
- **Performance Optimized** - Fast loading with global CDN delivery
- **SEO Ready** - Optimized for search engines
- **Direct Video Section Links** - Share direct URLs to specific video categories
- **Instant Page Loading** - No loading screens, immediate access to content
- **Smooth Navigation** - Pages always start from top viewport

## 🛠️ Tech Stack

- **Frontend**: React 18, TypeScript, Vite
- **Styling**: Tailwind CSS
- **Icons**: Lucide React
- **Video Hosting**: GitHub + jsDelivr CDN (FREE!)
- **Deployment**: Vercel/Netlify ready

## 🆓 Free Video Hosting Solution

This project uses a completely free video hosting solution:
- **GitHub repository** for video storage
- **jsDelivr CDN** for global delivery
- **No bandwidth limits** or costs
- **Unlimited traffic** and fast worldwide access

## 🔧 Setup & Installation

### Prerequisites
- Node.js 18+
- GitHub account (for video hosting)

### Local Development

1. **Clone the repository**
   ```bash
   git clone https://github.com/spydercreations-git/Idyllproductions.git
   cd Idyllproductions
   ```

2. **Install dependencies**
   ```bash
   npm install
   ```

3. **Run the development server**
   ```bash
   npm run dev
   ```

### Video Setup

1. **Follow the guide**: See `FREE_VIDEO_SETUP.md` for complete instructions
2. **Create video repository**: Set up a separate GitHub repo for videos
3. **Upload videos**: Add your compressed videos to the video repo
4. **Update URLs**: Videos will automatically work via CDN

## 🚀 Deployment

### Vercel (Recommended)
```bash
npm i -g vercel
vercel --prod
```

### Netlify
```bash
npm run build
# Upload dist/ folder to Netlify
```

**No environment variables needed** - everything is free and public!

## 📖 Documentation

- `FREE_VIDEO_SETUP.md` - Complete free video hosting guide
- `DEPLOYMENT.md` - Deployment guide

## 🎥 Video Categories

- **Short-Form Content** - Reels, Shorts, TikTok (`/short-form`)
- **Long-Form** - Extended content and storytelling (`/long-form`)
- **SaaS & Tech Videos** - Product demos, explainers (`/saas-tech`)
- **Gaming Content** - Gaming highlights and montages (`/gaming`)
- **Rhythmic Montage** - Cinematic video editing (`/rhythmic-montage`)

### Direct Access URLs
You can share direct links to specific video categories:
- `https://yoursite.com/short-form` - Short-form content
- `https://yoursite.com/long-form` - Long-form videos
- `https://yoursite.com/saas-tech` - SaaS & tech videos
- `https://yoursite.com/gaming` - Gaming content
- `https://yoursite.com/rhythmic-montage` - Rhythmic montage

## 🔒 Security Features

- Content protection (no right-click, dev tools disabled)
- Video download protection
- Environment variable protection

## 📱 Browser Support

- Chrome 90+
- Firefox 88+
- Safari 14+
- Edge 90+

## 💰 Cost

**$0.00** - Completely free forever!
- Free GitHub hosting
- Free jsDelivr CDN
- Free Netlify/Vercel deployment
- No hidden costs or limits

## 🤝 Contributing

1. Fork the repository
2. Create a feature branch
3. Make your changes
4. Test thoroughly
5. Submit a pull request

## 📄 License

This project is proprietary and confidential. All rights reserved.

## 📞 Contact

- **Website**: [Idyll Productions](https://idyllproductions.com)
- **Email**: idyllproductionsofficial@gmail.com
- **Instagram**: [@idyllproductionsofficial](https://instagram.com/idyllproductionsofficial)

---

Built with ❤️ by Idyll Productions | 100% Free Solution